...
export async function POST(req) {
  try {
    ...
    if (userResult.rows.length === 0) {
      ...
      return Response.json({ success: true, message: 'If that email exists, an OTP has been sent.' })
    }

    // Rate limit
    const recentCheck = await pool.query(
      'SELECT created_at FROM otp_store WHERE email = $1 ORDER BY id DESC LIMIT 1',
      [email]
    )

    if (recentCheck.rows.length > 0) {
      const secondsAgo = (Date.now() - new Date(recentCheck.rows[0].created_at).getTime()) / 1000
    ...
    // Store in DB (same table as send-otp)
    ...
    await transporter.sendMail({
      ...
      to: email,
      subject: 'Reset Password OTP - IntelliSIM',
      html: `
        <div style="font-family: Arial; padding: 20px; max-width: 400px;">
          ...
          <p style="color: #666;">Valid for 5 minutes. Do not share this with anyone.</p>
